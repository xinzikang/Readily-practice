...
//				console.log()
		...
		//在拖拽的时候,为了保证有抬起事件，所以将dragover阻止
		$(document).on('dragover',function(){
			return false;
		});
		
		if(this.opt.dragBox){
			//抬起的时候，将file文件push到数组中
			this.opt.dragBox.on('drop',function(ev){
				_this.addArr(ev.originalEvent.dataTransfer.files[0])
//				console.log(ev.originalEvent.dataTransfer.files[0]);
				return false;
			});
			//移入的时候触发
			this.opt.dragBox.on('dragenter',function(){
				_this.opt.ondragenter();
			});
